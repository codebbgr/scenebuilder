...
public class MavenRepositorySystem {
        ...
        MavenPresets.getPresetRepositories().stream()
            .filter(
                r ->
                    !onlyReleases
                        || (onlyReleases
                            && !r.getId().toUpperCase(Locale.ROOT).contains("SNAPSHOT")))
            .map(this::createRepository)
            .collect(Collectors.toList());
    list.addAll(
        repositoryPreferences.getRepositories().stream()
            .filter(
                r ->
                    !onlyReleases
                        || (onlyReleases
  ...
  private void cleanMetadata(Artifact artifact) {
    final String path =
        localRepo.getBasedir().getAbsolutePath()
            + File.separator
            + artifact.getGroupId().replaceAll("\\.", Matcher.quoteReplacement(File.separator))
            + File.separator
            + artifact.getArtifactId()
            + File.separator;
    final DefaultMetadata metadata =
        new DefaultMetadata("maven-metadata.xml", Metadata.Nature.RELEASE);
        ...
        sha1Files.stream()
            .forEach(
                f -> {
                  try {
                    FileUtils.copyFile(
                        f, new File(jarFile.getParent() + File.separator + f.getName()));
                  } catch (IOException ex) {
                  }
                });
      }
  ...
  public String resolveDependencies(RemoteRepository remoteRepository, Artifact artifact) {
    ...
    collectRequest.setRepositories(
        remoteRepository == null ? getRepositories() : Arrays.asList(remoteRepository));
  ...
  private RemoteRepository createRepository(Repository repository) {
    ...
    final RemoteRepository repo =
        new RemoteRepository.Builder(repository.getId(), repository.getType(), repository.getURL())
            .setSnapshotPolicy(
                onlyReleases ? new RepositoryPolicy(false, null, null) : new RepositoryPolicy())
