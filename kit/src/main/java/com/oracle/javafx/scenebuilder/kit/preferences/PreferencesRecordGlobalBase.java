...
public abstract class PreferencesRecordGlobalBase {
  ...
  public void readFromJavaPreferences() {
    ...
    // Document size
    final double height =
        applicationRootPreferences.getDouble(PreferencesControllerBase.ROOT_CONTAINER_HEIGHT, -1);
    setRootContainerHeight(height);
    final double width =
        applicationRootPreferences.getDouble(PreferencesControllerBase.ROOT_CONTAINER_WIDTH, -1);
    setRootContainerWidth(width);

    // Background image
    final String image =
        applicationRootPreferences.get(
            PreferencesControllerBase.BACKGROUND_IMAGE, DEFAULT_BACKGROUND_IMAGE.name());
    setBackgroundImage(BackgroundImage.valueOf(image));

    // Alignment guides color
    final String agColor =
        applicationRootPreferences.get(
            PreferencesControllerBase.ALIGNMENT_GUIDES_COLOR,
    ...
    final String prColor =
        applicationRootPreferences.get(
            PreferencesControllerBase.PARENT_RING_COLOR, DEFAULT_PARENT_RING_COLOR.toString());
    setParentRingColor(Color.valueOf(prColor));

    // Document theme
    String themeName =
        applicationRootPreferences.get(PreferencesControllerBase.THEME, DEFAULT_THEME.name());
  ...
  public void writeToJavaPreferences(String key) {
    ...
    switch (key) {
      case PreferencesControllerBase.ROOT_CONTAINER_HEIGHT:
        applicationRootPreferences.putDouble(
            PreferencesControllerBase.ROOT_CONTAINER_HEIGHT, getRootContainerHeight());
        break;
      case PreferencesControllerBase.ROOT_CONTAINER_WIDTH:
        applicationRootPreferences.putDouble(
            PreferencesControllerBase.ROOT_CONTAINER_WIDTH, getRootContainerWidth());
        break;
      case PreferencesControllerBase.BACKGROUND_IMAGE:
        applicationRootPreferences.put(
            PreferencesControllerBase.BACKGROUND_IMAGE, backgroundImage.name());
        break;
      case PreferencesControllerBase.ALIGNMENT_GUIDES_COLOR:
        applicationRootPreferences.put(
            PreferencesControllerBase.ALIGNMENT_GUIDES_COLOR, getAlignmentGuidesColor().toString());
        break;
      case PreferencesControllerBase.PARENT_RING_COLOR:
        applicationRootPreferences.put(
            PreferencesControllerBase.PARENT_RING_COLOR, getParentRingColor().toString());
        break;
  ...
  private static Image getImage(BackgroundImage bgi) {
    ...
    switch (bgi) {
      ...
      case BACKGROUND_02:
        url =
            PreferencesRecordGlobalBase.class.getResource("Background-Neutral-Grid.png"); // NOI18N
        break;
      case BACKGROUND_03:
