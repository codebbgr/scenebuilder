...
public class DocumentWindowController extends AbstractFxmlWindowController {
  ...
  private final CssPanelDelegate cssPanelDelegate =
      new CssPanelDelegate(inspectorPanelController, this);
  private final CssPanelController cssPanelController =
      new CssPanelController(editorController, cssPanelDelegate);
  private final LibraryPanelController libraryPanelController =
      new LibraryPanelController(
          editorController, PreferencesController.getSingleton().getMavenPreferences());
  ...
  private final SceneStyleSheetMenuController sceneStyleSheetMenuController =
      new SceneStyleSheetMenuController(this);
  private final CssPanelMenuController cssPanelMenuController =
      new CssPanelMenuController(cssPanelController);
  ...
  private final DocumentWatchingController watchingController =
      new DocumentWatchingController(this);
  ...
  private final EventHandler<KeyEvent> mainKeyEventFilter =
      event -> {
        ...
        if (isTextInputControlEditing(focusOwner) && accelerator != null) {
          ...
          //                              // When using system menu bar, the event is handled
          // natively
          //                              // before the application receives it : we just consume
          // the event
          //                              // so the editing action is not performed a second time by
          // the app.
  ...
  public DocumentWindowController() {
    super(
        DocumentWindowController.class.getResource("DocumentWindow.fxml"), // NOI18N
        I18N.getBundle(),
        false); // sizeToScene = false because sizing is defined in preferences
  ...
  public void loadFromFile(File fxmlFile) throws IOException {
    ...
    WarnThemeAlert.showAlertIfRequired(
        editorController, editorController.getFxomDocument(), getStage());
  }
  ...
  public boolean canPerformControlAction(DocumentControlAction controlAction) {
    ...
    switch (controlAction) {
      ...
      case SAVE_FILE:
        result =
            isDocumentDirty()
                || editorController.getFxomDocument().getLocation()
                    == null; // Save new empty document
        break;
      ...
      case REVERT_FILE:
        result = isDocumentDirty() && editorController.getFxomDocument().getLocation() != null;
        break;

      case REVEAL_FILE:
        result =
            (editorController.getFxomDocument() != null)
    ...
    switch (controlAction) {
      ...
      case SHOW_SAMPLE_CONTROLLER:
        if (skeletonWindowController == null) {
          skeletonWindowController =
              new SkeletonWindowController(
                  editorController,
                  Utils.makeTitle(editorController.getFxomDocument()),
                  getStage());
  ...
  protected void controllerDidLoadFxml() {
    ...
    messageBarController
        .getSelectionBarHost()
        .getChildren()
        .add(selectionBarController.getPanelRoot());

    inspectorSearchController
        .textProperty()
        .addListener(
            (ChangeListener<String>)
                (ov, oldStr, newStr) -> inspectorPanelController.setSearchPattern(newStr));

    librarySearchController
        .textProperty()
        .addListener(
            (ChangeListener<String>)
                (ov, oldStr, newStr) -> libraryPanelController.setSearchPattern(newStr));
    ...
    librarySplitController =
        new SplitController(libraryDocumentSplitPane, SplitController.Target.FIRST);
    documentSplitController =
        new SplitController(libraryDocumentSplitPane, SplitController.Target.LAST);
    ...
    getEditorController()
        ...
        .addListener(
            (ChangeListener<Number>)
                (ov, t, t1) -> messageBarController.setDocumentDirty(isDocumentDirty()));
    ...
    libraryLabel
        ...
        .bind(
            Bindings.createStringBinding(
                () -> {
                  return SceneBuilderApp.getSingleton().getUserLibrary().isExploring()
                      ? I18N.getString("library.exploring")
  ...
  public boolean isFrontDocumentWindow() {
    return getStage().isFocused()
        ...
        || (jarAnalysisReportController != null
  ...
  public void performCloseFrontDocumentWindow() {
    if (getStage().isFocused()) {
      performCloseAction();
    } else if (previewWindowController != null && previewWindowController.getStage().isFocused()) {
  ...
  void onHierarchyShowNodeId(ActionEvent event) {
    hierarchyPanelController.setDisplayOption(
        AbstractHierarchyPanelController.DisplayOption.NODEID);
    documentAccordion.setExpandedPane(documentAccordion.getPanes().get(0));
  ...
  public void onManageJarFxml(ActionEvent event) {
    if (libraryDialogController == null) {
      libraryDialogController =
          new LibraryDialogController(
              editorController,
              AppSettings.getUserM2Repository(),
              AppSettings.getTempM2Repository(),
              PreferencesController.getSingleton(),
              getStage());
      libraryDialogController.setOnAddJar(
          () -> onImportJarFxml(libraryDialogController.getStage()));
      libraryDialogController.setOnEditFXML(
          fxmlPath -> {
      ...
      libraryDialogController.setOnAddFolder(
  ...
  void onLibraryShowJarAnalysisReport(ActionEvent event) {
    if (jarAnalysisReportController == null) {
      jarAnalysisReportController =
          new JarAnalysisReportController(getEditorController(), getStage());
      jarAnalysisReportController.setToolStylesheet(getToolStylesheet());
    }
  ...
  private void performImportMedia() {
    ...
    final ExtensionFilter imageFilter =
        new ExtensionFilter(
            I18N.getString("file.filter.label.image"), ResourceUtils.getSupportedImageExtensions());
    final ExtensionFilter audioFilter =
        new ExtensionFilter(
            I18N.getString("file.filter.label.audio"), ResourceUtils.getSupportedAudioExtensions());
    final ExtensionFilter videoFilter =
        new ExtensionFilter(
            I18N.getString("file.filter.label.video"), ResourceUtils.getSupportedVideoExtensions());
    final ExtensionFilter mediaFilter =
        new ExtensionFilter(
            I18N.getString("file.filter.label.media"), ResourceUtils.getSupportedMediaExtensions());
  ...
  /**
   * Returns true if the specified node is part of the main scene and is either a TextInputControl
   * or a ComboBox.
   *
  ...
  /**
   * Returns true if we are editing within a popup window : either the specified node is showing a
   * popup window or the inline editing popup is showing.
   *
   * @param node the focused node of the main scene
   * @return
   */
  ...
  public void refreshFromPreferencesRecordGlobal(
      PreferencesRecordGlobal recordGlobal, boolean refreshTheme) {
  ...
  private void addCssPanelSearchListener() {
    cssPanelSearchController
        .textProperty()
        .addListener(
            (ChangeListener<String>)
                (ov, oldStr, newStr) -> cssPanelController.setSearchPattern(newStr));
  }
      ...
      try {
        ...
        if (saveConfirmed) {
          try {
            ...
            var recordGlobal = getPreferencesRecordGlobal();
            final byte[] fxmlBytes =
                editorController
                    .getFxmlText(recordGlobal.isWildcardImports())
            ...
            editorController
                .getMessageLog()
                .logInfoMessage("log.info.save.confirmation", I18N.getBundle(), fileName);
    ...
    if (editorController.canGetFxmlText()) {
      ...
      if (fxmlFile == null) {
        ...
        if (!path.endsWith(".fxml")) { // NOI18N
          try {
            URL alternateURL =
                new URL(fxmlFile.toURI().toURL().toExternalForm() + ".fxml"); // NOI18N
            ...
            if (alternateFxmlFile.exists()) {
              details +=
                  "\n" // NOI18N
                      + I18N.getString(
                          "alert.save.noextension.details.overwrite", alternateFxmlFile.getName());
            }
